...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setupOptionButtons('term-options', (val) => { state.termStatus = val; calculateRespiratoryScore(); });
...
function calculateRespiratoryScore() {
    ...
    if (!isNaN(spo2)) {
        if (state.chdType === 'cyanotic') {
            ...
            const threshold = state.termStatus === 'preterm' ? 90 : 95;
            if (spo2 < threshold) {
                spo2Score = 3;
            }
        }
    ...
    const threshold = state.termStatus === 'preterm' ? 90 : 95;
    let spo2CriteriaText = state.chdType === 'cyanotic' ? 'SpO₂ < 75%' : `SpO₂ < ${threshold}%`;
    const cyanoticNote = isCyanoticSevere ? ' <span style="color:#d97706; font-weight:bold;">(Cyanotic CHD + SpO₂ < 75%)</span>' : '';

    state.details.resp = `
        <p><strong>ข้อมูลที่ระบุ:</strong> RR: ${rr||'-'}, SpO2: ${spo2||'-'}%, CHD: ${state.chdType||'ไม่มี'}, Condition: ${state.termStatus === 'preterm' ? 'Preterm' : 'Term'}</p>
        <hr style="margin:0.5rem 0;">
